...
def should_substitute(path, relative_path, search_regex, used_dep_set, used_dip_set):
    """
...
def compute_lists_proc(path, source_tree, domain_search_regex, url_search_regex):
    """
    ...
    if os.path.isfile(path):
        ...
        if path.is_symlink():
            ...
            try:
                ...
                if should_substitute(path, relative_path, domain_search_regex, used_dep_set,
                                              used_dip_set):
                    domain_substitution_set.add(relative_path.as_posix())
                if should_substitute(path, relative_path, url_search_regex, used_dep_set,
                                              used_dip_set):
                    url_substitution_set.add(relative_path.as_posix())
    ...
    return (used_pep_set, used_pip_set, used_dep_set, used_dip_set, pruning_set,
            domain_substitution_set, url_substitution_set, symlink_set)


def compute_lists(source_tree, domain_search_regex, url_search_regex, processes):
    """
    ...
    with Pool(processes) as procpool:
        returned_data = procpool.starmap(
            compute_lists_proc,
            zip(source_tree.rglob('*'), repeat(source_tree), repeat(domain_search_regex), repeat(url_search_regex)))

    # Handle the returned data
    for (used_pep_set, used_pip_set, used_dep_set, used_dip_set, returned_pruning_set,
         returned_domain_sub_set, returned_url_sub_set, returned_symlink_set) in returned_data:
        ...
        url_substitution_set.update(returned_url_sub_set)
    ...
    return sorted(pruning_set), sorted(domain_substitution_set), sorted(url_substitution_set), unused_patterns
...
def main(args_list=None):
    ...
    parser.add_argument(
        ...
        metavar='PATH',
        type=Path,
        default='url_substitution.list',
        help='The path to store url_substitution.list. Default: %(default)s')
    parser.add_argument(
        '--url-regex',
        metavar='PATH',
        type=Path,
        default='url_regex.list',
        help='The path to url_regex.list. Default: %(default)s')
    parser.add_argument(
        '-t',
        '--tree',
        metavar='PATH',
    ...
    pruning_set, domain_substitution_set, url_substitution_set, unused_patterns = compute_lists(
        args.tree,
        DomainRegexList(args.domain_regex).search_regex, 
        DomainRegexList(args.url_regex).search_regex, args.processes)
    ...
    with args.url_substitution.open('w', encoding=_ENCODING) as file_obj:
        file_obj.writelines('%s\n' % line for line in url_substitution_set)
