...
void ChromeImporter::StartImport(const importer::SourceProfile& source_profile,
  ...
  if (set_encryption_key && (items & importer::PASSWORDS) && !cancelled()) {
...
void ChromeImporter::ImportBookmarks() {
  ...
  if (!bookmarks.empty() && !cancelled()) {
      const std::u16string& first_folder_name =
        base::UTF8ToUTF16(std::string("Imported from ")) + importer_name_;
  ...
  if (folder->GetList("children", &children)) {
    for (const auto& value : children->GetList()) {
      ...
      if (type == "folder") {
        ...
        const base::ListValue* children;
        if (dict->GetList("children", &children) && children->GetList().empty()) {
...
void ChromeImporter::ImportPasswords() {
  ...
  success = database.GetBlocklistLogins(&blacklist);
  if (success) {
