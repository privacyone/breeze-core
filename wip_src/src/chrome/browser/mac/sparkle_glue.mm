...
- (void)registerWithSparkle {
  ...
  DCHECK(breezesparkle::UpdateEnabled());
  DCHECK(su_updater_);
  ...
  constexpr NSTimeInterval kBreezeUpdateCheckIntervalInSec = 6 * 60 * 60;
  [su_updater_ setUpdateCheckInterval:kBreezeUpdateCheckIntervalInSec];
...
- (void)updater:(id)updater didFinishLoadingAppcast:(id)appcast {
  VLOG(0) << "Breeze update: did finish loading appcast";

  [self updateStatus:kAutoupdateChecking version:nil error:nil];
}

- (void)updater:(id)updater didFindValidUpdate:(id)item {
  VLOG(0) << "Breeze update: did find valid update with " +
...
- (void)updaterDidNotFindUpdate:(id)updater {
  VLOG(0) << "Breeze update: did not find update";
...
- (void)updater:(id)updater
  ...
  VLOG(0) << "Breeze update: willDownloadUpdate with " +
             GetDescriptionFromAppcastItem(item);
...
- (void)updater:(id)updater
    failedToDownloadUpdate:(id)item
                     error:(NSError *)error {
  VLOG(0) << "Breeze update: failed to download update with " +
             GetDescriptionFromAppcastItem(item) +
...
- (void)userDidCancelDownload:(id)updater {
  VLOG(0) << "Breeze update: user did cancel download";
  [self updateStatus:kAutoupdateInstallFailed
               version:nil
                 error:nil];
}
...
- (void)updater:(id)updater willInstallUpdate:(id)item {
  VLOG(0) << "Breeze update: will install update with " +
             GetDescriptionFromAppcastItem(item);
  [self updateStatus:kAutoupdateInstalling
             version:nil
               error:nil];
...
- (void)updater:(id)updater
  ...
  VLOG(0) << "Breeze update: will install update on quit with " +
             GetDescriptionFromAppcastItem(item);
...
- (void)updater:(id)updater didAbortWithError:(NSError *)error {
  VLOG(0) << "Breeze update: did abort with error: " +
