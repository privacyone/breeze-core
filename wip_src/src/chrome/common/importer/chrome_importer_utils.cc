...
base::ListValue* GetChromeSourceProfiles(const base::FilePath& user_data_folder) 
{
  ...
  else 
  {
    ...
    if (local_state_dict->GetDictionary("profile", &profile_dict)) 
    {
      ...
      {
        for (base::DictionaryValue::Iterator it(*info_cache); !it.IsAtEnd(); it.Advance()) 
        {
...
bool ChromeImporterCanImport(const base::FilePath& profile, uint16_t* services_supported) 
{
