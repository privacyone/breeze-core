...
def get_breeze_version():
    """Returns the Chromium version."""
    f = (Path(__file__).parent.parent / 'breeze_version.txt').read_text().splitlines()
    version =  f[0].split('=',1)[1] + '.'
    version =  version + f[1].split('=',1)[1] + '.'
    version =  version + f[2].split('=',1)[1] + '.'
    return(version + f[3].split('=',1)[1])
